...
Claude Code에서 `/senior-mode` 또는 고정 진입점(`/senior-mode:codex`, `:luna`, `:deepseek`, `:team`)을 입력해서 켭니다. 그러면 Claude가 컴패니언 스크립트를 알아서 몰아 줍니다.

이 스킬은 **명시적 호출 전용**입니다(`disable-model-invocation: true`). 위임 조사가 도움이 될 법한 작업을 그냥 설명하는 것만으로는 켜지지 않습니다 — 켜고 끄는 타이밍을 사용자가 쥐고 있어야 위임 비용이 예측 가능하기 때문입니다. 스크립트는 스킬 없이 직접 돌릴 수도 있습니다.
